...
class Bot(ABC):
    ...
    def chatbox_text_RED_slayer_bracelet(self, contains: str = None) -> Union[bool, str]:
        if contains is None:
            return ocr.extract_text(self.win.chat_first_line, ocr.PLAIN_12, clr.RED)
        if ocr.find_text(contains, self.win.chat_first_line, ocr.PLAIN_12, clr.RED):
            return True
    ...
    def chatbox_text_QUEST_large(self, contains: str = None) -> Union[bool, str]:
    ...
    def chatbox_text_ABSORPTION(self, contains: str = None) -> Union[bool, str]:
        if contains is None:
            return ocr.extract_text(self.win.chat, ocr.PLAIN_12, clr.ABSORPTION_POTION)
        if ocr.find_text(contains, self.win.chat, ocr.PLAIN_12, clr.ABSORPTION_POTION):
            return True
        
    def chatbox_text_ANTIFIRE(self, contains: str = None) -> Union[bool, str]:
        if contains is None:
            return ocr.extract_text(self.win.chat, ocr.PLAIN_12, clr.ANTIFIRE)
        if ocr.find_text(contains, self.win.chat, ocr.PLAIN_12, clr.ANTIFIRE):
            return True
