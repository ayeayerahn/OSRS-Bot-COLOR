...
class OSRSardyknights(OSRSBot):
        ...
        while time.time() - start_time < end_time:
            ...
            while True:
                if self.chatbox_text_BLACK_first_line(contains="continue"): # Full coin pouch
                    ...
                    self.mouse.click()
                    break
                elif self.chatbox_text_BLACK_first_line(contains="rogue") or self.chatbox_text_BLACK_first_line(contains="left"):
                    #self.log_msg("Success")
                    break
                elif self.chatbox_text_BLACK_first_line(contains="stunned") or self.chatbox_text_BLACK_first_line(contains="fail"): # failure messages
                    #self.log_msg("Fail")
                    self.check_hp()
                    time.sleep(4)
                    break
        ...
        while True:
            if count < 10:
                ...
                if self.mouseover_text(contains="Pickpocket"):
                    ...
                    try: self.mouse.move_to(knight.random_point()) 
                    except: AttributeError
                    count += 1
                    time.sleep(1)
                    if count == 10:
                        self.log_msg("Tried to move to knight and failed")
                        self.stop()  
                    continue
            else:
                count += 1
    ...
    def open_bank(self):
        ...
        dodgy_neck_bank_img = imsearch.BOT_IMAGES.joinpath("Aarons_images", "dodgy_necklace_bank.png")
        dodgy_neck_img = imsearch.BOT_IMAGES.joinpath("Aarons_images", "dodgy_necklace.png")
        count = 0
        while True:
            if count < 10:
        ...
        if bank_text := ocr.find_text("Bank Bank", self.win.game_view, ocr.BOLD_12, [clr.WHITE, clr.CYAN]):
            ...
            self.mouse.click()
        shark = self.wait_until_img(shark_bank_img, self.win.game_view) # Wait until raw sharkies are seen
        dodgy_neck_bank = self.wait_until_img(dodgy_neck_bank_img, self.win.game_view) # Wait until raw sharkies are seen
        time.sleep(1)
        
        if dodgy_neck := imsearch.search_img_in_rect(dodgy_neck_img, self.win.control_panel):
            self.mouse.move_to(dodgy_neck.random_point())
            self.mouse.right_click()
            if deposit_text := ocr.find_text("All", self.win.control_panel, ocr.BOLD_12, [clr.WHITE, clr.ORANGE]):
                self.mouse.move_to(deposit_text[0].random_point(), knotsCount=0)
                self.mouse.click()
            
        self.mouse.move_to(dodgy_neck_bank.random_point())
        if self.mouseover_text(contains="Release"):
            ...
            self.mouse.right_click()
            if withdraw_text := ocr.find_text("6", self.win.game_view, ocr.BOLD_12, [clr.WHITE, clr.ORANGE]):
